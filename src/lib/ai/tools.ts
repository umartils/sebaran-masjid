...
export const masjidTools = {
  ...
  getDaftarMasjid: tool({
    ...
    inputSchema: z.object({
      namaProvinsi: z.string().optional().describe('Nama provinsi, contoh: "Jawa Barat"'),
      namaKota: z.string().optional().describe('Nama kota/kabupaten, contoh: "Bandung"'),
      kategori: z
        .enum(['Pelosok_Pedalaman', 'Muslim_Minoritas', 'Kampung_Mualaf', 'Terdampak_Bencana'])
        .optional()
      ...
      limit: z.number().min(1).max(20).default(10).describe('Jumlah maksimum hasil'),
    }),
    execute: async ({ namaProvinsi, namaKota, kategori, kondisi, limit }) => {
      const masjidList = await prisma.masjid.findMany({
        where: {
          ...
          ...(namaProvinsi && { namaProvinsi: { contains: namaProvinsi, mode: 'insensitive' } }),
          ...(namaKota && { namaKota: { contains: namaKota, mode: 'insensitive' } }),
          ...(kategori && { kategori }),
          ...(kondisi && { kondisi }),
        },
      ...
      if (masjidList.length === 0) {
        return { found: false, message: 'Tidak ada masjid yang cocok dengan kriteria tersebut.' };
      }
  ...
  getDetailMasjid: tool({
    ...
    inputSchema: z.object({
      nama: z.string().optional().describe('Nama masjid, boleh sebagian (partial match)'),
      masjidId: z.string().optional().describe('ID masjid jika sudah diketahui'),
    }),
    execute: async ({ nama, masjidId }) => {
      if (!nama && !masjidId) {
        return { found: false, message: 'Nama atau ID masjid harus disebutkan.' };
      }
      ...
      if (!masjid) {
        return { found: false, message: `Masjid "${nama ?? masjidId}" tidak ditemukan.` };
      }
  ...
  getProgresMasjid: tool({
    ...
    inputSchema: z.object({
      ...
      masjidId: z.string().optional().describe('ID masjid jika sudah diketahui'),
      jumlahLog: z.number().min(1).max(10).default(5).describe('Jumlah log terbaru yang diambil'),
    }),
  ...
  getLaporanProgresPdf: tool({
    ...
    inputSchema: z.object({
      ...
      semuaLaporan: z
        .boolean()
        .default(false)
        .describe('true jika user ingin semua laporan mingguan dalam satu dokumen, false untuk laporan terbaru saja'),
    }),
    execute: async ({ nama, masjidId, semuaLaporan }) => {
      ...
      return {
        ...
        hasReport: true,
        nama: masjid.nama,
        label: semuaLaporan ? 'Semua Laporan Progres' : 'Laporan Progres Terbaru',
        downloadUrl,
      };
    },
  }),
